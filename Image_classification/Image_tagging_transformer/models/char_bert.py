...
class BertCharacterEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.word_embeddings = CharacterCNN(
            requires_grad=True,
            output_dim=config.token_embedding)
...
class CharacterBertModel(nn.Module):
    """ BertModel using char-cnn embeddings instead of wordpiece embeddings. """

    def __init__(self, bert_config='bert-large-uncased'):
        super().__init__()
        self.config = BertConfig(bert_config)

        self.embeddings = BertCharacterEmbeddings(self.config)
