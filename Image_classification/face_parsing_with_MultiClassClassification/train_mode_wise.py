...
# Local files
from model import BaseFeatureExtractor, UniqueFeature
...
def parse_args():
    ...
    parser.add_argument('--dataDir',
                        help='data directory',
                        type=str,
                        default='data/scrapped_filtered')
...
def main():
    ...
    logger, tb_log_dir = create_logger(opts, phase = opts.train_mode)
    ...
    assert opts.train_mode in ['hair', 'beard', 'mustache'], "Please enter Training Phase"
    ...
    if opts.train_mode == 'hair':
        model = UniqueFeature(base_model = base_model, classes=18)

    elif opts.train_mode == 'beard':
        model = UniqueFeature(base_model = base_model, classes = 9)
    
    elif opts.train_mode == 'mustache':
        model = UniqueFeature(base_model = base_model, classes = 8)
    ...
    data = SingleFeatureDataset(mode = opts.train_mode, root = opts.dataDir, transform= trans)
    ...
    train_loader = DataLoader(trainset, batch_size=opts.batch_size, shuffle=True, pin_memory=True, num_workers=8)
    ...
    for epoch in range(opts.epochs):
        ...
        if mats['accuracy'] > val_acc:
            ...
            ckpt.update(mats)
            torch.save(ckpt, '{}/{}_{}_best_model.pt'.format(opts.modelDir, opts.model_type, opts.train_mode))
